...
public abstract
class YoixAWTTextComponent extends YoixAWTCanvas
...

{
    ...
    protected
    YoixAWTTextComponent(YoixObject data, YoixBodyComponent parent) {
    ...
    disposeSavedGraphics(Graphics g) {
        ...
        if (savegraphics && savedgraphics == null)
            savedgraphics = g;
        else g.dispose();
    }
    ...
    getSavedGraphics() {
        ...
        //
        // Clipping originally was only done in YoixAWTTextTerm, but
        // it seems to help some subclasses without breaking others
    ...
    setSaveGraphics(boolean state) {
        ...
        if (state != savegraphics) {
            savegraphics = state && (ISWIN9X == false || YoixMisc.jvmCompareTo("1.3") < 0);
            disposeSavedGraphics();
        }
    }
